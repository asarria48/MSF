...
public:
  ...
  double feq(double rho, double Jx0, double Jy0, int i); //Función de equilibrio a partir de los campos y de la dirección i
...
double LatticeBoltzman::feq(double rho0, double Jx0, double Jy0, int i){  //dada la densidad y las corrientes, calcula la función de equilibrio dada una dirección i 
...
void LatticeBoltzman::Start(double rho0, double Jx0, double Jy0){
  ...
  for(ix = 0; ix < Lx; ix++)
    for(iy = 0; iy < Ly; iy++)
      ...
      for(i = 0; i < Q; i++){
	n0 = n(ix,iy,i);
	f[n0] = feq(rho0,Jx0,Jy0,i); //Obligo que el valor de la función sea el valor de equilibrio que se obtiene de los campos macroscópicos, inicializo no a cero sino a los valores de equilibrio predeterminados 
      }
}
...
void LatticeBoltzman::Collision(void){
  ...
  for(ix = 0; ix < Lx; ix++){
	...
	fnew[n0] = UmUtau*f[n0]+Utau*feq(rho0,Jx0,Jy0,i); //Ecuación de Boltzmann discretizada (fórmula que nos dan)
      }
   }
 }
